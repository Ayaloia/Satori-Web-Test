...
export function SkewedCard({children, className, rotateMultiple = 10}: { children: React.ReactNode, className?: string | undefined , rotateMultiple?: number}) {
    ...
    return (
        ...
        <motion.div ref={ref} className={`w-full h-full ${className}`} initial={{transform: "perspective(700px) rotateX(0) rotateY(0)"}}
                    whileHover={{transformOrigin: "center", transform: `perspective(700px) rotateX(${(elY / elH - 0.5) * rotateMultiple}deg) rotateY(${(elX / elW - 0.5) * -rotateMultiple}deg)`, transition: {duration: 0}}}>
            {children}
        </motion.div>
    )
}
    ...
    return (
        <header className="p-3 flex h-full w-full flex-col items-center">
            <div ref={ref} className="h-[425vh] flex flex-col items-center">
                <Image className="z-10 object-cover" src="/satoriTop.jpg" alt="satori" fill={true} sizes="100vw"/>
...
export function SatoriIntroduce() {
    return (
        <section className="flex justify-evenly flex-wrap gap-5">
            <motion.article className="bg-gray-300 rounded-2xl p-4 md:w-2/3 shadow-lg space-y-5 font-serif"
                ...
                <section className="space-y-3">
                    ...
                    <p>
                        ...
                        然而，她却很受无法说话的动物们所欢迎。因为读心的能力，对于动物来说可以说是独一无二的能力。就因为这样，地灵殿栖息着很多动物<span
                        className="text-gray-500">（×1
                ...
                <section className="space-y-3">
                    ...
                    <p>管理着灼热地狱迹的她所身处的别墅。地板还装有暖气。只有古明地姐妹住在这个宽敞得要命的地方<span
                        className="text-gray-500">（×2
            ...
            <motion.figure
                ...
                <div className="w-full aspect-[292/612] object-contain relative">
                    <Image src="/satori(kos).png" alt="satori" fill={true} sizes="(max-width: 768px) 50%, 20%"
